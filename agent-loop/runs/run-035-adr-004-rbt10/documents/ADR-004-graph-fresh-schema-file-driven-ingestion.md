...
Instantiation of authoritative-source ground truth (the selection-catalog and standards planes) is subject to the fidelity control the ground-truth-mutation governance record commits: a **provisional-versus-official distinction**, where captured representations enter provisionally and **official, consumable entry is gated behind a human fidelity verification** that the captured representation faithfully reflects its authoritative source. That control's existence, its provisional-to-official shape, and its scope (the authoritative-source class, per-write) are the governance record's; this ADR **realizes that control for instantiation** and is the record the governance record names as the home of the ingestion mechanics. Two commitments follow here: the gate is scoped to the authoritative-source class — it is not applied to the ungated observational stream or to externally-decided mirrored records; and **read-exclusion of provisional ground truth is a schema-level invariant** — a provisional authoritative-source node is excluded from ground-truth synthesis until officially entered, homed in the schema's read-discipline invariant set and mirrored by the port's behavioral contract. The port-level mechanics — how provisional state is represented and how the official-entry transition is enforced — are the knowledge-service design's.
...
- The Reasoning Graph is safe by construction against the instantiation path — no data-load operation can write, reconstruct, or discard it (its durability across instance loss rests on instance-level backup and restore, whose production posture is deferred — §2.5, §5.3).
...
- Authoritative-source ground truth enters provisionally and reaches official, consumable state only through a human fidelity verification; provisional nodes are read-excluded from synthesis until then; the observational stream and mirrored externally-decided records are not gated.
...
3. **Additive-not-destructive.** Instantiation and re-ingestion supersede (on a version increment) or add no version (on unchanged content); no design introduces a destructive-reset or wipe operation in the instantiation path.
...
6. **Fidelity gate.** Authoritative-source ground truth reaches official, consumable state only through a human fidelity verification, and provisional nodes are read-excluded from synthesis until then; the observational stream and mirrored externally-decided records are not gated.
...
- **DDR-002 (Graph Schema)** — the supersession discipline instantiation is additive under (DDR-002 §6), the never-delete and read-discipline invariants including the fidelity-gate read-exclusion home (DDR-002 §7), and the plane-definition and declaration model source artifacts validate against (DDR-002 §2.6).
