...
|---|---|---|---|
| **Promotion** — Reasoning-Graph findings into authoritative selection knowledge (Catalog / Standards) | The existing EA promotion gate | Per-write (EA) | Built — the durable chain: ADR-001 §2.5 (EA-gated-promotion principle) + DDR-002 §5 (`PromotionDecision` gate) / §7 #15 (promoted-origin invariant) |
...
| **Environment** — CMDB-observed deployed reality | Per-policy capture-time-frozen aging (freshness-decayed confidence derived at capture, never recomputed at read) under DDR-004 §6 calibration governance; **no** per-write gate | Per-policy | Form built (DDR-004); constants contested/unvalidated (n=0) |
...
**Provenance survives mutation.** The traceable rationale of a decision that consumed a ground-truth fact must survive that fact's later mutation. A design that relied on a promoted fact which is later retracted must remain answerable by traversal to the fact as it stood when it was consumed; the frozen-provenance survival guarantee is currently scoped to promoted facts (§5.3), so a design that consumed a fact without promoting it relies on Reasoning-Graph evidence under bounded retention. This is the reasoning-capture auditability commitment (ADR-001) applied to the mutation domain: withdrawal removes a fact from *use*, not from the *record* of what informed prior decisions.
...
- **The forthcoming feedback-loop governance design** — downstream consumer of this ADR; owns the remedy boundary and all governance policy (§2.6). This ADR is its upstream authority; the obligation to resolve that record's pointer to this record's concrete document identifier lives in that record's definition of done.
