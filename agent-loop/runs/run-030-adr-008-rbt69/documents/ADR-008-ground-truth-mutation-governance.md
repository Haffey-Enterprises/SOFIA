...
|---|---|---|---|
| **Promotion** — Reasoning-Graph findings into authoritative selection knowledge (Catalog / Standards) | The existing EA promotion gate (ADR-001 §2.5 EA-gated-promotion principle) | Per-decision-act (EA), batch-capable | Built — the graph-schema ruling's `PromotionDecision` gate into Catalog/Standards (DDR-002 §5 / §7 #15) and its promoted-origin invariant |
...
| **Operational distillation** — a derived `ObservedPattern` into the Operational plane | Substantive, evidence-visible human review of the **distinct distilled judgment**, bounded by distinctness | Per-distinct-lesson | The forthcoming feedback-loop governance design; the detection-promotion service design |
| **Environment** — CMDB-observed deployed reality | Per-policy capture-time-frozen aging (the inherited-confidence derivation ruling's derivation, calibration-governed under its §6); **no** per-write gate | Per-policy | The inherited-confidence derivation ruling's aging basis and its §6 calibration governance (DDR-004) |
...
- **Environment.** CMDB-observed reality is continuous and high-volume; a per-write human gate would make the plane unwritable at observation cadence. Its control is per-policy capture-time-frozen aging: the plane ages by construction — freshness-decayed confidence derived at capture and frozen, never recomputed at read (its aging *form* is fixed in the inherited-confidence derivation ruling; its constants are contested and unvalidated at zero capture traffic under that ruling's §6) — and it is never consumed as authoritative *selection* knowledge. A human is answerable for the weighting policy (its calibration is a ratified act under that ruling's governance), not for each observation.

### 2.3 Un-promotion authority

Un-promotion — retraction of a fact that was promoted into authoritative selection knowledge — is the **withdrawal-leg of the promotion class**, governed by the same per-write, EA-gated, human-accountable control *class* as promotion itself — the **authority** is homed here on the same footing; the governing-verdict detection analogue for executed retractions is the named follow-up (DDR-002 §7 / DDR-003), since the built retraction instance does not yet supply promotion's governing-verdict guarantee (see this subsection's trailing clause and §5.3). This ADR is the **upstream authority** for un-promotion: ADR-001 §2.5 authorizes promotion only and does not stretch to its reversal. The built retraction shape — the EA-gated reversing `CandidatePromotion` with its `RETRACTS` edge (the graph-schema ruling's §5 and its retraction-gated invariant) — is the instance of this authority; the built retraction invariant traces to *an* approving decision, not the *governing* one, so it does not yet supply promotion's governing-verdict guarantee — that governing-verdict detection analogue for executed retractions is the named follow-up (§5.3). Un-promotion is **never a bare delete**: the withdrawn node is read-excluded from ground-truth synthesis and retained, fully traceable, for audit (§2.4).
...
- **Operational distillation will no longer be an unchecked derived write.** A distilled lesson must pass a substantive, evidence-visible human review before it is consumable track-record, realized through a forthcoming, additive candidate-`ObservedPattern` shape — introduced downstream by a governance design and schema amendment, not a present schema constraint — that will exclude it from ground-truth traversal until approved.
...
- **The forthcoming feedback-loop governance design** — downstream consumer of this ADR; owns the remedy boundary and all governance policy (§2.6). This ADR is its upstream authority; the obligation to resolve its pointer to this record's concrete document identifier lives in that design's definition of done.
...
- **Deliberation substrate** — the RBT-59 pre-authoring deliberation record (dispositions ratified per item, 2026-07-17); the triage-001 charter (Appendix A); and Reboot Decision Ledger ruling R30 (the derive/promote authority seam), fetched verbatim as substrate.
