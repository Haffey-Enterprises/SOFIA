...
**Propagation.** Propagation of this commitment across reasoning-architecture design work is carried by the compliance checks above, enforced at three-hat review; no dedicated propagation directive is required. This is distinct from runtime constraint propagation, whose question is answered by ADR-003 (Platform Patterns): enterprise constraints bind agents from the Knowledge Graph (ADR-003 §2.4), and the formerly forthcoming Directives-Context-Envelope Bridge is superseded by that ruling — no bridge mechanism is forthcoming.
...
- **ADR-003 (Platform Patterns)** — answers the runtime directive-propagation question the formerly forthcoming Directives-Context-Envelope Bridge carried: enterprise constraints bind agents from the Knowledge Graph (ADR-003 §2.4; transport-carried directive propagation rejected at its §4.3). No bridge mechanism is forthcoming.
...
|---|---|---|---|
| 1.4.0 | 2026-07-20 | RBT-9 | Pointer resolution — the §6 Propagation clause and the §7 cross-reference to the "Directives-Context-Envelope Bridge (forthcoming, unauthored)" resolve: **ADR-003 (Platform Patterns), ACCEPTED v1.0.0** in this batch, answers the runtime directive-propagation question via its constraints-from-graph ruling (§2.4), with transport-carried directive propagation rejected (its §4.3); no bridge mechanism is forthcoming (RBT-11 disposition: folded — supersession-by-principle; ruled at the RBT-9 deliberation, Item 7). Marker-level; no decision change. |
