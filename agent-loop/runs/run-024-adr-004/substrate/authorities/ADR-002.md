...
**Enforcement.** These checks are enforced at three-hat (LAA/SA/EA) review — the gate every SDD and DDR passes through — at design time. The mechanizable store-authority and write-authority checks (more mechanizable than judgment-based review) are carried at the schema layer by the graph conformance harness, which mechanizes the DDR-002 §7 invariant set those checks map onto; a catch-up increment extends that coverage. Until mechanization is complete, the review-judgment checks here remain aspirational — conformance reviewed at SDD/DDR-authoring time and at three-hat architectural reviews.
...
|---|---|---|---|
| 1.2.0 | 2026-07-15 | RBT-63 | **Contract-purity (no decision change):** the §6 Enforcement paragraph is de-drifted to subject-named form — the inline operational identifiers (the `conformance/` harness path, its RBT-33 Increment 1, and the RBT-48 catch-up increment) are removed from normative prose per the docs-structure validator's contract-purity rule; the harness and its catch-up increment stay named by subject in §6, and the concrete pointers live here in the Change Log. A pre-existing latent violation (introduced in the 1.1.0 triage batch, PR #8) that the diff-scoped validator surfaced only when this amendment re-touched the file. |
