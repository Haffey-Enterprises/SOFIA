...
| **References** | DDR-002 v1.5.0 (§4 canonical confidence definition + inheritance rule + #24 ceiling + `Evidence.observed_at` capture-instant semantic, §1 contested-T2 convention, §2.1–2.5 + §2.6 plane schemas incl. the declared-basis carriers); DDR-001 v1.4.0; ADR-001 v1.2.0 (§2.2 source-attribution intent, §5.2 read discipline); ADR-002 v1.1.0; SDD-001 v1.4.0 (§3.4.3 derivation execution, §4.6 config surface, §9 cross-reference) |
...
- **Extension `PlaneDefinition`** — **basis 2** (flat base: versioned registry ground truth; staleness is supersession). The registration contract that carries every plane's `confidence_basis` (§2.6) is itself a versioned authoritative class — a superseded registry version is read-excluded, not decayed; no freshness term.

**Honest floor:** citation reachability is unverified for the basis-2 classes with no designed Evidence-citation role — `DeploymentEnvironment`, `Governance`-plane nodes, and `PlaneDefinition` — as is the non-citability assumption for the basis-4 cost inputs `RateCard`/`CostFactor`. The fail-closed backstop keeps behavior safe regardless; the dispositions above resolve the permanent rule per class.
...
- **Implements:** DDR-002 v1.5.0 §4 (canonical confidence definition, inheritance rule, #24 ceiling), §1 (contested-T2 convention), §2.1–2.5 + §2.6 (core-plane and Extension plane schemas, carrying this record's basis declarations); ADR-001 v1.2.0 §2.2 (source-attribution intent).
- **Implemented by:** SDD-001 v1.4.0 (`knowledge-service` — the gateway that computes the derivation) §3.4.3, §4.6, §9; the forthcoming solutioning-agent SDD (the `ReasoningProgress` rollup and per-target composition, §5).
...
|---|---|---|---|
| 1.4.0 | 2026-07-18 | RBT-68 | **Pin-currency re-pin sweep — no decision change.** References + Cross-References pins refreshed to the versions the RBT-59 / ADR-008 pointer-resolution batch produced: ADR-001 v1.1.0 → v1.2.0, DDR-001 v1.3.0 → v1.4.0, DDR-002 v1.4.0 → v1.5.0, SDD-001 v1.3.0 → v1.4.0 (Implements: DDR-002, ADR-001; Implemented-by: SDD-001). The **ADR-002 v1.1.0** pin is deliberately left as-is — the ADR-002 v1.1.0 → v1.2.0 currency + SDD-001 §4.6 runtime realignment is a separate, partly pre-existing cascade tracked on RBT-66 (operator-ruled split at RBT-62 review, finding F-C), not folded here. No content the pins carry changed; a pure currency touch does not restale sibling pins (so this v1.4.0 bump does not re-open the RBT-59-version targets). No ruling, schema, or contract change. MINOR per the corpus no-patch-species convention. |
