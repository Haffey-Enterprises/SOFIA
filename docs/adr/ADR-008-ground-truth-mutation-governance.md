...
| **Authoritative-source ingestion** — Catalog / Standards | Pre-entry human **fidelity** verification of the captured representation, gating official (consumable) entry | Per-write | ADR-004 (authority — resolved); the knowledge-service ingestion port (mechanics) |
...
- **ADR-004 (Graph as Fresh Schema / File-Driven Ingestion), ACCEPTED v1.0.0** — the record the "forthcoming file-driven ingestion decision record" routing resolves to. The discharge is deliberately partial per that record's own ruling: ADR-004 homes the ingestion-mechanics *authority* realizing the authoritative-source ingestion control (§2.2); the port-level mechanics land at the knowledge-service ingestion port (its design's ingestion-port contract).
...
|---|---|---|---|
| 1.1.0 | 2026-07-19 | RBT-14 | Pointer resolution — the body and cross-reference subject-name routings to "the forthcoming feedback-loop governance design" resolve to **DDR-003 (Feedback Loop Governance), ACCEPTED v1.0.0**, landed in this batch. Marker-level; no decision change. Also resolves one stale "forthcoming feedback-loop governance" body marker missed at v1.1.0. |
| 1.2.0 | 2026-07-22 | RBT-10 | Pointer resolution — the "forthcoming file-driven ingestion decision record" routings (§2.2 control-home row; §7 cross-reference) resolve to **ADR-004 (Graph as Fresh Schema / File-Driven Ingestion), ACCEPTED v1.0.0**; discharge deliberately partial per ADR-004's ruling (authority there; port mechanics at the knowledge-service ingestion port). Marker-level; no decision change. Also resolves one stale "forthcoming feedback-loop governance" body marker missed at v1.1.0. |
