...
The SOFIA graph is instantiated **greenfield from declared source artifacts**, and instantiation — schema provisioning and data load alike — is orchestrated by a **driver-less loader utility that leverages knowledge-service's ports**. knowledge-service remains the **sole holder of the graph driver** for data-definition and data writes both; no other component holds a graph driver, and knowledge-service is committed here to expose a schema-provisioning capability it does not yet enumerate (§2.2). There is **no migration**: the graph is authored fresh, prior-corpus material is reference-only and never a starting point, and the instantiation path has **no destructive-reset mode**. Instantiation writes the Knowledge Graph only and preserves the Reasoning Graph by construction. Read-exclusion of provisional authoritative-source ground truth from ground-truth synthesis until officially entered is committed as a forthcoming additive amendment to the schema record's read-discipline invariant set, firing with the fidelity-gate implementation (§2.7). Two obligations are enumerated as decisions of this record: the knowledge-service design gains the schema-provisioning administrative operation with provisioning-before-readiness ordering, and the constraint-versioning ownership deferral homes on that same forthcoming design amendment (§2.2, §5.3).
...
Instantiation of authoritative-source ground truth (the Catalog and Standards planes) is subject to the fidelity control the ground-truth-mutation governance record commits: a **provisional-versus-official distinction**, where captured representations enter provisionally and **official, consumable entry is gated behind a human fidelity verification** that the captured representation faithfully reflects its authoritative source. That control's existence, its provisional-to-official shape, and its scope (the authoritative-source class, per-write) are the governance record's; this ADR **realizes that control for instantiation** and is the record the governance record names as the home of the ingestion-mechanics *authority* — the decision that authority is routed to, distinct from the port-level mechanics that realize it. That forward reference's discharge is deliberately partial: authority resolves here; the mechanics resolve at the knowledge-service ingestion port. Two commitments follow here: the gate is scoped to the authoritative-source class — it is not applied to the ungated Environment observation stream, nor to records whose deciding authority is external, mirrored into the graph as captured references per the schema record's externally-decided `GateDecision` construct (DDR-002 §2.4): such records are not products of ingestion fidelity and sit outside the gate's class. The Operational distillation class is separately governed, not exempt — it carries its own review control and is out of scope here; and **read-exclusion of provisional ground truth is committed as a schema-level invariant** — a provisional authoritative-source node is excluded from ground-truth synthesis until officially entered. That invariant is a *forthcoming* addition to the schema's read-discipline invariant set (a named amendment obligation), not one the schema already carries, mirrored by the port's behavioral contract. The port-level mechanics — how provisional state is represented and how the official-entry transition is enforced — are the knowledge-service design's.
...
- Authoritative-source ground truth enters provisionally and reaches official, consumable state only through a human fidelity verification; provisional nodes are read-excluded from synthesis until then. Not gated: the ungated Environment observation stream, and externally-decided mirrored records (captured references per the schema record's `GateDecision` construct, DDR-002 §2.4 — not products of ingestion fidelity). Separately governed, not exempt: Operational distillation, which carries its own review control outside this gate's scope.
...
- **Constraint-versioning ownership over the platform's life is deferred.** Provisioning is governed by construction (a knowledge-service operation), but how it evolves a live instance's constraints over time is not settled here. *Mitigation:* the operation's location in the sole owner keeps any future evolution on the governed path; the versioning policy is a named forward question homed on the same forthcoming knowledge-service-design amendment as the provisioning capability (§2.2), not a silent gap.
...
6. **Fidelity gate.** Once the forthcoming read-discipline invariant and port mechanics land (§5.3), authoritative-source ground truth reaches official, consumable state only through a human fidelity verification, and provisional nodes are read-excluded from synthesis until then; the ungated Environment observation stream and externally-decided mirrored records (per the schema record's `GateDecision` construct) are not gated, and Operational distillation is separately governed by its own review control.
...
- **ADR-008 (Ground-Truth Mutation Governance)** — the authoritative-source fidelity control this ADR realizes for instantiation (ADR-008 §2.2, §5.2); this record is the "forthcoming file-driven ingestion decision record" that governance record names as the home of ingestion-mechanics authority — a deliberately partial discharge: authority resolves here, port mechanics at the knowledge-service ingestion port.
