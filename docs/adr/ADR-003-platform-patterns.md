...
- **Substrate services** — components that execute, enforce, store, or mediate without authoring. The knowledge-service gateway is the canonical instance (sole executor of graph writes, author of none); the model-access gateway is of this class (infrastructure facade — model access, no judgment of record).
...
- **DDR-003 (Feedback Loop Governance)** — governance policy for the promotion path; the routing of predicate-evaluation semantics onward to the forthcoming constraint-validator service design.
...
- **The forthcoming file-driven ingestion decision record** — the Standards-plane entry path on which §2.4's enforcement stages depend.
